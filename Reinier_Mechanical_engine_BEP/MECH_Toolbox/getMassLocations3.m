...
function [points,comPoint] = getMassLocations3(DNA)
...
for k = 1:nM                                            %for all masses
    ...
    if numel(edgeIdx)==1                                %If there is only one edge
        ...
        elseif edgelabel==3        %for prismatic joint
            Pidx = sum(DNA.edgelabel(1:edgeIdx)==edgelabel);
            points{k}(2,:) = DNA.Ppar(1:2,Pidx).';
            
        elseif edgelabel==4
            Pmidx = sum(DNA.edgelabel(1:edgeIdx)==edgelabel);
            points{k}(2,:) = DNA.Pmpar(1:2,Pmidx).';
