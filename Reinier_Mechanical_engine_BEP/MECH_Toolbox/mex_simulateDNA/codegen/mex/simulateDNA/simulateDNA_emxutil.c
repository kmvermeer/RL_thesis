 ...
 *
 * simulateDNA_emxutil.c
 *
 * Code generation for function 'simulateDNA_emxutil'
 *
 */
...
void emxFreeMatrix_cell_wrap_01(cell_wrap_0 pMatrix[2])
{
  int32_T i;
  for (i = 0; i < 2; i++) {
    emxFreeStruct_cell_wrap_0(&pMatrix[i]);
...
void emxInitMatrix_cell_wrap_01(const emlrtStack *sp, cell_wrap_0 pMatrix[2],
  const emlrtRTEInfo *srcLocation, boolean_T doPush)
{
  int32_T i;
  for (i = 0; i < 2; i++) {
...
{
  emxInit_real_T(sp, &pStruct->incstr, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->edgelabel, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->Mpar, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->Hpar, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->Spar, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->Ppar, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->Pmpar, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->bestTrajectory, 2, srcLocation, doPush);
}
...
/* End of code generation (simulateDNA_emxutil.c) */
